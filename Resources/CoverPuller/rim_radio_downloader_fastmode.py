...
# -*- coding: utf-8 -*-
"""
Rim-Radio Cover Downloader - Fast Mode Build
Speed features:
- Connection pooling (requests HTTPAdapter) with retries
- Lower timeouts
- Adjustable thread pool + provider candidate limits
- Optional previews (skip thumbnails to speed selection)
- Auto-pick when confident (string similarity + year tolerance)
"""
...
UA = "RimRadioAlbumFetcher/fast-mode"
REQ_TIMEOUT = 8  # seconds
DEFAULT_PAR = 12  # higher default parallelism
DEFAULT_LIMIT = 10  # max per provider
DEFAULT_YEAR_TOL = 1
...
class ChoiceDialog(tk.Toplevel):
    def __init__(self, parent, artist, album, year, candidates, show_previews=True, thumb_pool=4):
        super().__init__(parent)
        self.title("Choose album cover (Fast)")
    ...
    def _populate_cards(self):
        ...
        for i, c in enumerate(self.candidates):
            ...
            if self.show_previews and PIL_OK:
                # lazy-load thumbnails in a thread
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Rim-Radio Cover Downloader - Fast")
        ...
        self.show_previews = tk.BooleanVar(value=False)  # off by default for speed
        self.auto_pick = tk.BooleanVar(value=True)       # auto-pick when confident

        # providers
        self.use_itunes = tk.BooleanVar(value=True)
        self.use_deezer = tk.BooleanVar(value=True)
        self.use_mb = tk.BooleanVar(value=False)  # off by default - slower
    ...
    def _build_ui(self):
        ...
        self.tree = ttk.Treeview(mid, columns=("genre","artist","album","year","status"), show="headings", height=16)
        for col, w in [("genre",120), ("artist",220), ("album",360), ("year",80), ("status",480)]:
        ...
        self.log = tk.Text(right, height=18, wrap="word"); self.log.pack(fill="both", expand=True)
        ...
        ttk.Label(bottom, textvariable=self.status_var, width=18).pack(side="left", padx=(8,0))
        ttk.Button(bottom, text="Load CSV", command=self.load_csv).pack(side="right", padx=(8,0))
        ttk.Button(bottom, text="Start", command=self.start).pack(side="right", padx=(8,0))
    ...
    def start(self):
        ...
        self.status_var.set("Running"); self.progress_var.set(0); self.append_log("Starting download...")
        limit = self.limit_var.get() or 0
        self.worker = threading.Thread(target=self._worker_thread, args=(out, limit), daemon=True)
        self.worker.start(); self.after(120, self._poll_queue)
    ...
    def choose_url_ui(self, artist, album, year, candidates):
        dlg = ChoiceDialog(self, artist, album, year, candidates, show_previews=self.show_previews.get(), thumb_pool=max(4, self.parallelism.get()//2))
    ...
    def _maybe_autopick(self, artist, album, year, candidates):
        # choose the candidate with best title similarity if >= 0.92 and within year tol
        if not candidates:
            return None
    ...
    def _worker_thread(self, out_dir, limit):
        total = len(self.rows) if limit == 0 else min(limit, len(self.rows))
        done = 0
        for idx, r in enumerate(self.rows[:total]):
            if self.stop_flag.is_set():
                self.queue.put((self.append_log, (f"Stopped by user at item {idx+1}/{total}.",))); break

            artist = r["Artist"]; album = r["Album"]; year = (r["Year"].strip() if r["Year"] else None)
            genre = r["Genre"]

            def rowlog(msg): self.queue.put((self.append_log, (f"[{artist} - {album}] {msg}",)))
            ...
            try:
                while True:
                    ...
                    # Auto-pick when confident and Always-ask is off
                    url = None
                    if self.auto_pick.get() and not self.ask_always.get():
                    ...
                    fname = f"{slug(genre)}__{slug(artist)}__{slug(album)}__{slug(str(yr_int) if yr_int else 'NA')}{ext}"
